...
def adjust_learning_rate(optimizer, original_lr=1e-4, decay_coef=0.95):
    for param_group in optimizer.param_groups:
        param_group['lr_pi'] = original_lr * decay_coef


def run_vpg_train(env, agent, max_time_steps, logger, eval_interval, num_evals):
    time_step_counter = 0
    ...
    while True:
        ...
        for i in range(0, agent.steps_per_iter, env.num_envs):
            ...
            if not agent.discrete_action:
                ...
                logpi = agent.compute_logpi(mus, logsigmas, sigmas, actions)
                mus = mus.cpu().numpy()
        ...
        assert mb_obs.ndim <= 2 and mb_rewards.ndim <= 2 and mb_actions.ndim <= 2 and \
               mb_logpi_old.ndim <= 2 and mb_dones.ndim <= 2 and mb_obs_.ndim <= 2, \
        ...
        adjust_learning_rate(agent.optimizer_pi, original_lr=agent.lr_pi, decay_coef=decay_coef)
        if agent.value_func_type is not None:
            adjust_learning_rate(agent.optimizer_v, original_lr=agent.value_lr, decay_coef=decay_coef)
        ...
        print("episode_len:".ljust(20) + "{:.1f}".format(np.mean([epi_info['length'] for epi_info in epi_info_buf])))
